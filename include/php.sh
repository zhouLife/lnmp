...
PHP_with_Sodium()
{
    if [ "${Enable_PHP_Sodium}" = "n" ]; then
        ...
        # ===== 此处增加了对 15(8.4) 和 16(8.5) 选项原生支持 sodium 的判定 =====
        if echo "${PHPSelect}" | grep -Eqi "^[8-9]|1[0-6]$" || echo "${php_version}" | grep -Eqi '^7.[2-4].*|[8-9].*' || echo "${Php_Ver}" | grep -Eqi "php-7.[2-4].*|php-[8-9].*"; then
